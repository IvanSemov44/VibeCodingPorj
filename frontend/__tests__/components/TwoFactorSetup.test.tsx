...
vi.mock('qrcode', async () => {
  return {
    default: {
      toCanvas: (canvas: HTMLCanvasElement, uri: string, opts: any, cb: (err?: unknown) => void) => cb(),
    },
  };
});
...
describe('TwoFactorSetup', () => {
  ...
  test('shows loading state', () => {
    (useGet2faSecretQuery as unknown as any).mockReturnValue({ data: null, error: null, isLoading: true, refetch: vi.fn() });
    (useEnable2faMutation as unknown as any).mockReturnValue([vi.fn(), { isLoading: false, error: null }]);
  ...
  test('renders secret and provisioning uri when data present', () => {
    ...
    (useGet2faSecretQuery as unknown as any).mockReturnValue({ data, error: null, isLoading: false, refetch: vi.fn() });
    (useEnable2faMutation as unknown as any).mockReturnValue([vi.fn(), { isLoading: false, error: null }]);
  ...
  test('show enable button when no data and clicking triggers enable', async () => {
    ...
    (useGet2faSecretQuery as unknown as any).mockReturnValue({ data: null, error: null, isLoading: false, refetch: vi.fn() });
    (useEnable2faMutation as unknown as any).mockReturnValue([enableFn, { isLoading: false, error: null }]);
