...
describe('lib/api basic smoke tests', () => {
  ...
  test('login posts credentials', async () => {
    ...
    const postCall = calls.find((args: unknown) => (Array.isArray(args) ? (args[1] as Record<string, unknown>)?.method === 'POST' : false));
  ...
  test('uploadToolScreenshots sends FormData body', async () => {
    ...
    const uploadCall = mockFetch.mock.calls.find((c: unknown[]) => typeof c[0] === 'string' && String(c[0]).includes('/screenshots')) as unknown[] | undefined;
    const body = uploadCall ? (uploadCall[1] as Record<string, unknown> | undefined)?.body : undefined;
    // body should be a FormData-like instance (has append)
    expect(Boolean(body) && typeof ((body as unknown) as { append?: unknown }).append === 'function').toBe(true);
