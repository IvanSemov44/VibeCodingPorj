...
describe('lib/api unit tests', () => {
  ...
  it('login returns an auth user when credentials are correct', async () => {
    ...
    const authObj = auth as { user?: { id?: number } } | undefined;
    expect(authObj?.user).toBeDefined();
    expect(authObj?.user?.id).toBe(1);
  });
  ...
  it('uploadToolScreenshots sends FormData and returns screenshots list', async () => {
    ...
    try {
      ...
      const resObj = res as { screenshots?: unknown } | undefined;
      expect(Array.isArray(resObj?.screenshots)).toBe(true);
      expect((resObj?.screenshots as unknown[] | undefined)?.length ?? 0).toBeGreaterThanOrEqual(0);
    } finally {
